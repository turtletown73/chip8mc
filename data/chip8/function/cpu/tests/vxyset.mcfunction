...
execute unless score .temp math matches 10 run tellraw @a [{"text":"Failure: output is not 10, but is ",color:red},{"score":{name:".temp",objective:"math"},color:red}]
execute unless score .temp math matches 10 run data modify storage chip8:emu testpass set value 0

execute if data storage chip8:emu {testpass:1} run tellraw @a {"text":"Test vxyset passed! ",color:green}